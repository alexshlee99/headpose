import cv2
import numpy as np
from math import pi,atan2,asin, cos, acos, sqrt
...
class PoseEstimator:
    ...
    def solve(self, points):
        ...
        if self.r_vec is None:
            (_, rotation_vector, translation_vector) = cv2.solvePnP(
                self.model_points_68, points.astype('float32'), self.camera_matrix, self.dist_coeefs)
    ...
    def euler_angles(self, pose):
        ...
        P = np.hstack((R, t_vec))
        euler_angle = cv2.decomposeProjectionMatrix(P)[-1]  # OpenCV uses XYZ-order. 
        pitch, yaw, roll = euler_angle.flatten()
